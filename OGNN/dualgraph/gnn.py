...
class GNN2(nn.Module):
    ...
    def forward(self, batch, perturb=None, perturb_edge=None):
        ...
        if node_batch is None or node_batch.numel() == 0:
            print("Warning: node_batch is None or empty. Skipping GNN processing for this batch.")
            # If the batch is empty, return a tensor of appropriate shape for an empty batch.
            # The output of self.gnn(g) in MedModel is [B, 128].
            # If B (batch size) is 0, we need to return a [0, 128] tensor.
            # Use x.device to ensure it's on the correct device.
            return torch.empty(0, self.latent_size, device=x.device) # Return an empty tensor
                                                                    # with the correct feature dimension
        else:
            num_graphs = node_batch.max().item() + 1

        # Now, ensure graph_idx is correctly handled.
        # This part should be safe now because we've handled the empty batch case above.
        graph_idx = torch.arange(num_graphs, device=x.device)
